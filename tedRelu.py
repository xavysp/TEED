...
class _DenseLayer(nn.Sequential):
    def __init__(self, input_features, out_features):
        ...
        self.add_module('af1', nn.ReLU(inplace=True)), # Smish()
    ...
    def forward(self, x):
        ...
        new_features = super(_DenseLayer, self).forward(F.relu(x1))  # F.relu() Fsmish
...
class UpConvBlock(nn.Module):
    ...
    def make_deconv_layers(self, in_features, up_scale):
        ...
        for i in range(up_scale):
            ...
            layers.append(nn.ReLU(inplace=True)) # Smish()
...
class SingleConvBlock(nn.Module):
    def __init__(self, in_features, out_features, stride, use_ac=False):
        ...
        self.conv = nn.Conv2d(in_features, out_features, 1, stride=stride,
                              bias=True)
        if self.use_ac:
            self.af = nn.ReLU(inplace=True)#Smish() nn.ReLU(inplace=True)
...
class DoubleConvBlock(nn.Module):
    def __init__(self, in_features, mid_features,
        ...
        self.af= nn.ReLU(inplace=True) # Smish()#
